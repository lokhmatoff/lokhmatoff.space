---
title: Laravel
description: Заметки о топовом PHP-фреймворке для веб-ремесленников
date: 2024-11-21
uid: 202411211200
draft: false
---

PHP-фреймворк для веб-ремесленников с кучей батареек, понятной [документацией](https://laravel.com/docs) и практически бесконечной расширяемостью.

Возможно, самый популярный фреймворк для [[PHP]] на текущий момент. Явно самый простой способ начать разработку веб-приложений на PHP. Один из моих основных инструментов в бэкенд-разработке!
