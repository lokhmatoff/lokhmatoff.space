...
> [!success] Пофиксили
> Если дата открытия этой заметки — 15 октября или позже, значит, **[[Omarchy|Omarchy 3.1.0]]** уже вышла в свет, и использует она `walker` версии `2.0.0`. Начиная с этой версии дистрибутива проблема исправлена!

Для начала коротко рассказываю о `walker`. Это многоцелевой лаунчер для приложений и скриптов для [[tags/linux|Linux]]-систем, наподобие `drun`, `rofi` и подобных. Волкер стоит как лаунчер по умолчанию в [[Omarchy|Omarchy]] по умолчанию (запуск приложений, *Omarchy Meny*, диалоговое окно для блокировки/перезагрузки/скринсейвера/выключения).
